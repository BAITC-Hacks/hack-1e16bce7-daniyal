...
Страница входа, хранение токена в браузерной сессии и подключение кабинетов — задача frontend.

## Расхождения с текущим frontend

Frontend-контракт в `docs/frontend-api.md` был подготовлен отдельно и пока ожидает
`POST auth/demo` с role, вложенный user в ответе auth/me и массивы вместо страниц.
Backend реализует согласованный план выше: отдельные employee/hr маршруты,
обязательный пароль HR, прямой ответ auth/me и объекты items/total.
Для подключения нужно адаптировать frontend-прокси, форму HR-входа и чтение коллекций.
Навыки возвращают level/assessed_level; required_level и readiness появятся с траекторией.
В текущем backend-этапе frontend не изменялся.
