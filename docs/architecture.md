...
соединение с БД, расчёт навыков и интерфейсы. Автономный AI-слой реализован; контракт описан в [инструкции интеграции](ai-integration.md). На / доступно пробное задание с localStorage; серверный сценарий прохождения ещё не реализован. Локальный результат не является оценкой сотрудника.
...
reason_codes и дополнительные факты уровней, истории и readiness. Выбор и баллы определяет
только Python. Формула v1 и tie-break зафиксированы в [контракте AI](ai-integration.md).
...
Личность сотрудника в OpenAI не передаётся. Реализован Responses-адаптер с ограниченной
генерацией: модель выбирает план и варианты формулировок подтверждённых фактов.
Итоговый текст собирается локально. Есть deadline, проверка ответа и шаблонный fallback.
