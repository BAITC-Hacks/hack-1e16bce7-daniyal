...
соединение с БД, расчёт навыков и интерфейсы. Автономный AI-слой реализован;
контракт описан в [инструкции интеграции](ai-integration.md).
Frontend подготовлен к серверному сценарию: профиль, траектория, рекомендации, completion, HR и импорт.
Вход, профиль, навыки и история подключены к текущим read-контрактам. Остальные
бизнес-API, включая подключение AI, ещё не реализованы; интеграционный контракт
и ограничения: [frontend-api](frontend-api.md).
