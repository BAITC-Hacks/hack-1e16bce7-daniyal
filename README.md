...
- `/` — профиль, серверная карьерная траектория, навыки, рекомендации с объяснениями,
  детали активности, завершение с результатом и история участия.
- `/hr` — KPI, gaps, статистика активностей, сотрудники с поиском/фильтрами,
  просмотр профиля и загрузка исходного/добавочного датасета.
...
Во время работы эти API появились в параллельных backend-изменениях. Траектория,
completion, HTTP-импорт, scoring, объяснения и HR-аналитика ещё ожидают API.
...
5. Войдите с demo-ролью после подключения бизнес-API. Устанавливать приложение не требуется.
...
Frontend в этом backend-этапе не изменяется.
