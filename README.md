...
- `/recommendations` — подбор обучения по профилям из PostgreSQL, объяснения через
  локальную Qwen3/Ollama или OpenAI и явный fallback на трёх языках.

Бизнес-таблицы, первая миграция и транзакционный CLI-импорт датасета реализованы.
Read-only API списка сотрудников и рекомендаций подключён к AI-слою.
Вход/JWT, операции изменения данных, добавочный импорт и HR-аналитика — следующие этапы.
