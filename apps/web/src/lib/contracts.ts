...
export type Event = { event_id: string; title: string; description: string; type: string; duration_hours?: number | null; develops_skills: Effect[] };
...
export type Activity = { record_id: string; event_id: string; title: string; status: ActivityStatus; date: string };
export type Page<T> = { items: T[]; total: number; limit: number; offset: number };
export type Completion = { event_id: string; already_completed: boolean; changes: { skill_id: string; name: string; before: number; after: number }[]; career_readiness_before: number | null; career_readiness_after: number | null };
export type Dashboard = { employee_count: number; average_readiness: number | null; without_recommendations: number; completed_activities: number; as_of_date: string };
