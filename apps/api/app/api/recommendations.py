"""Read-only AI recommendations scoped to the current employee or HR session."""
import asyncio
...
def employees(request: Request, response: Response, principal: PrincipalDependency):
    response.headers["Cache-Control"] = "no-store"
    try:
        with request.app.state.database.sessions() as session:
            query = select(Employee).order_by(Employee.employee_id).limit(500)
            if principal.role != Role.HR:
                query = query.where(Employee.employee_id == principal.employee_id)
...
async def recommendations(body: RecommendationRequest, request: Request, response: Response,
    ...
    response.headers["Cache-Control"] = "no-store"
    if principal.role != Role.HR and principal.employee_id != body.employee_id:
